...
void main() {           
    ...
    float arcDiff = max(dot(lightDir, normal), 0.0);
    vec3 diffuse = arcDiff * lightColor;

    // specular
    vec3 viewDir = normalize(viewPos - fs_in.FragPos);
    vec3 reflectDir = reflect(-lightDir, normal);
    float arcSpec = pow(max(dot(normal, reflectDir), 0.0), 32.0);
    vec3 specular = arcSpec * lightColor;    
